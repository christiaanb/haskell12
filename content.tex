...
\end{itemize}
\soosim has been released on Hackage.\footnote{Stable: \url{http://hackage.haskell.org/package/SoOSiM/}. Development: \url{http://github.com/christiaanb/SoOSiM}.}
...
An important requirement for \soosim, set forth by the \soos project, is that it should be able to straightforwardly simulate the instantiation of application threads and OS modules.
...
appear (hot-swap).
Hence, we also require that \soosim facilitates the straightforward creation and destruction of computing elements, represented by \emph{nodes} in \soosim.
The \soos project's current need for a simulator rests mostly in formalising the \soos concept, and examining the interaction between the envisioned OS modules and the application threads.
...
\begin{itemize}
  ...
  \item If a component is not blocked, and the message queue is empty, the component will be executed by passing an \emph{empty} message to it.
\end{itemize}
Empty messages facilitate component execution:
when desired, a component can inform the simulator that it does not want to receive these empty messages.
...
A component not making progress because it has an empty message queue, and having indicated that it does not want to receive \emph{empty} messages, is designated as \emph{idle}.
...
A value of type \hs{SimEvent} is either a message from another component or an empty message from the simulator:
\begin{code}
data SimEvent
...
\item {\hs{yield}}
informs the simulator that the component does not want to receive empty messages.
\item {\hs{readMem}}
...
To give a concrete example, using the system from Figure~\ref{fig:system} as our context: \emph{Thread \#6} wants to invoke the instance of the \emph{Memory Manager} that is running on the same \emph{Node \#2}.
As \emph{Thread \#6} was not involved with the instantiation of that OS module, it has no idea what the specific \hs{ComponentId} of the memory manager on \emph{Node \#2} is.
...
It knows the unique global name of the memory manager in question, so it can use the \hs{componentLookup} function to find the \hs{Memory Manager} with ID \emph{\#5} that is running on \emph{Node \#2}.
...
%format family="\textbf{\textup{family}}"
\begin{code}
type family Sem (m :: * -> *) a :: *
...
Trying to define a type class or a sum type for this setting would yield either a too large, or a too restrictive definition; trying to cater for every possible usage we would either overdo it, or limit  future applications.
