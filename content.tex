...
\end{itemize}
\soosim is also available from Hackage.\footnote{Issue \hs{cabal install SoOSiM} to install \soosim. See also:
...
newtype Coroutine m o i r 
  = Coroutine { resume :: m (Either (o, i 
     -> Coroutine m i' o r) r) }
\end{code}
we need to reference \hs{i'} on the left hand side. Using some more advanced tricks, like a type class for all the chain \hs{i}, \hs{i2}, \dots\ does not really help: all the types on the RHS still need to be listed on the LHS, thus exploding the definition.
