...
In the following we will show that using Haskell \cite{haskell-report}\cbcomment{I don't think we need to cite the haskell report for the haskell symposium} enabled us to implement this property in a nice way.
...
\olcomment{what is our story? what do we want to tell?}
...
Having both explicit messaging and shared memory,% \olcomment{memory or memories?}
...
The simulator monad \hs{SimM} is described in Figure~\ref{fig:code-simm} in Section~\ref{sec:impl-monads}.\olcomment{BAD}
...
the memory interaction are exactly studied, leaving out of consideration other parts of the language.\olcomment{wording!}
...
A monad can be defined with a Kleisli triple: combining a type constructor \hs{m}, a unit function \hs{return :: a -> m a}, and a bind function \hs{>>= :: m a -> (a -> m b) -> m b}, we obtain a monad.
...
We use dynamic types as `the other side'\olcomment{a reviewer does not like this!} of the type classes: we need to communicate heterogeneous types over a typed channel, and there is no option to know all the types-to-transmit at the moment of the channel definition.
...
Trying to define a type class or a sum type for this setting would yield either a too large\olcomment{explain!}, or a too restrictive definition.
