...
\begin{itemize}
\item To simulate message passing and shared memory\olcomment{memory or memories?} as the main forms of communication between software components.
...
In other words, if a blocking message is sent, the sender is waiting for an answer---regardless \emph{where} in the code of the sender the blocking \hs{invoke} method is called.
...
The type \hs{state} should be read as a placeholder for any (user-defined) datatype; there is no actual predefined \hs{state} datatype.
