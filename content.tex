...
Doing so has detrimental effects on the caches of the individual cores \cite{Temam:1994:CIP:183019.183047,Rafique:2006:ASO:1152154.1152160}.\olcomment{citations ok?}
...
In other words, if a blocking message is sent, the sender is waiting for the answer---regardless \emph{when}\olcomment{I stumbled against this again: when or where?} in the sender the message is sent.
...
All components in a simulated system, even those hosted within the same node, are executed concurrently.\olcomment{are meant to be executed concurrently? can be executed concurrently? or indeed \emph{are} executed concurrently?}
...
As an example we demonstrate a mini functional language with mutable references.\olcomment{was: "running example". IMO not the case!}
...
We can define most language constructs to behave like their haskell counterpart, e.g., the \hs{app} construct instance is defined using monadic bind (\textsf{$\gg$=}).
%($\mathit{>>=}$).
...
We utilise \textsf{Data.Dynamic} to convert transmitted values of a
