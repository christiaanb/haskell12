...
\olcomment{new: --- >}
The key point is: common operating systems' designs have multiple drawbacks. 
...
Heterogeneous (\eg GPU+CPU, Cell BE), non-coherent architectures (\eg Intel SCC) and network-on-a-chip approaches are quite poorly supported. 
...
In the following we will show that using Haskell\cbcomment{I don't think we need to cite the haskell report for the haskell symposium} enabled us to implement this property in a nice way.
...
Having both explicit messaging and shared memory,\olcomment{memory or memories?} \soosim supports the two well known methods of communication.
...
The type \hs{State} should be read as a place holder for any (user-defined) data type; there is no actual predefined \hs{State} data type.
...
For the above reasons we have decided to make use of embedded languages for the definitions of our applications.
...
To continue the execution of a resumeable computation we issue \hs{resume} \mbox{\hs{computation}.} %% hackery to get spaces right
