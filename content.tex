...
Moving to larger distributed systems, monolithic kernels hinder scalability as the entire internal state has to be synchronised amongst the copies.\cbcomment{I don't think we should say more about S(o)OS or its ideas / concepts in the intro.}
...
\olcomment{Do we need an overview of the whole project then?}\cbcomment{I don't think so}
...
\olcomment{is it clear, that FSM is standard approach, not coroutines?}\cbcomment{I hope the following subsection explains our use of coroutines better}
A \emph{naive} approach to model synchronization between OS modules, one we explored initially, is to implement an OS module as a finite state machine (FSM).
In state 'A' OS module 'Bob' could send a message to 'Alice' and enter state 'B'.
Only when receiving a response from 'Alice', would 'Bob' progress from state 'B' to state 'C'.
...
As a solution we essentially mechanized\cbcomment{correct wording?} the creation of the FSM using the notion of \emph{suspendable} computations.
...
We achieved suspendability using coroutines, more specifically, a Coroutine monad transformer.
...
As one could infer from Figure~\ref{fig:code-simm}, the implementation makes use of the coroutines~\cite{coroutines}.
...
We utilise \textsf{Control.Monad.Coroutine} to express coroutines.
...
A yet further option is to make the coroutine execution concurrent using actors~\cite{Hewitt:1973:UMA:1624775.1624804,sulzmann2008actors}.
